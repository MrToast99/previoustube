...
void countdown_mode::render() {
  ...
  auto &d = display_helper::get();
  char text[32]; // Increased to completely bypass format-truncation warning
...
void scoreboard_mode::render() {
  ...
  auto &d = display_helper::get();
  char text[32]; // Increased to prevent truncation warnings
...
void date_mode::update() {
  ...
  if (!show_day_name) {
    // Show MM.DD.YY
    char text[32]; // Increased to prevent GCC calculating 26 bytes and throwing an error
...
void temperature_mode::render() {
  ...
  char text[32]; // Bumping this as well for good measure
  if (negative) {
